...
def dashboard_view(request):
    if request.user.is_superuser:
        ...
        get_all_courses=Course.objects.all().count()
        get_all_program=Program.get_program_count()
        taken_course_count=TakenCourse.objects.all().count()
        context = {
            "student_count": User.get_student_count(),
            "lecturer_count": User.get_lecturer_count(),
            "superuser_count": User.get_superuser_count(),
            "Taken_course":taken_course_count,
            "all_courses":get_all_courses,
            "all_program":get_all_program,
        ...
        get_all_courses=Course.objects.all().count()
        get_all_program=Program.get_program_count()
        allocated_courses_count = CourseAllocation.get_course_allocated_count(request.user)
        context = {
            "student_count": User.get_student_count(),
            "lecturer_count": User.get_lecturer_count(),
            "superuser_count": User.get_superuser_count(),
            "course_count":get_all_courses,
            "program_count": get_all_program,
            "your_courses":allocated_courses_count,
            "Taken_course":TakenCourse.objects.all().count(),
            "logs": logs,
        }
        return render(request, "core/dashboard.html", context)
    else:
        logs = ActivityLog.objects.all().order_by("-created_at")[:10]
        get_all_courses=Course.objects.all().count()
        taken_course_count = TakenCourse.objects.filter(student__student=request.user).count()
        get_all_program=Program.get_program_count()
        gender_count = Student.get_gender_count()
        context = {
            "student_count": User.get_student_count(),
            ...
            "course_count":get_all_courses,
            "program_count": get_all_program,
            "your_courses":taken_course_count,
            "logs": logs,
            "males_count": gender_count["M"],
            "females_count": gender_count["F"],
        }
