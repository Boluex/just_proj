...
@receiver(post_save, sender=Course_feedback)
def log_save(sender, instance, created, **kwargs):
    verb = "created" if created else "updated"
    ActivityLog.objects.create(message=f"Someone made a comment on a course")


@receiver(post_delete, sender=Course_feedback)
def log_delete(sender, instance, **kwargs):
    ActivityLog.objects.create(message=f"A feedback has been deleted.")
...
class CourseAllocation(models.Model):
    ...
    @classmethod
    def get_course_allocated_count(cls, user):
        return cls.objects.filter(lecturer=user).count()
